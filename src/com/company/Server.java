...
public class Server {
    ...
    private Game game;
    private int port, opponentNumber = -1;
            ...
            while (running) {
                try {
                    ...
                    try{
                        ...
                        opponentNumber = Integer.parseInt(line);
                        System.out.println(opponentNumber + "    opp   " +game.getNumberOfPlayer());
                        if (!isTurnSeted && opponentNumber != -1) {                                                                                                                                                                                                                             //&& Integer.parseInt(game.getSideIPAdress().split("\\.")[3]) > Integer.parseInt(game.getIPAdressOfOpponent().split("\\.")[3])) {
                            if(game.getNumberOfPlayer() > opponentNumber) {
                                game.setPlayersTurn(true);
                                System.out.println(game.getNumberOfPlayer() + ">" + opponentNumber);
                                isTurnSeted = true;
                            }else if(game.getNumberOfPlayer() < opponentNumber) {
                                System.out.println(game.getNumberOfPlayer() + "<" + opponentNumber);
...
//                            isTurnSeted = true;
                    ...
                    System.out.println(line + "  " + Server.isTurnSeted);
                    if (isItIp) {
                        line.split(".");
                        if (!game.isSideIPDetected()) {
